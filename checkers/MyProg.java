...
//        memcpy(bestmove,state.movelist[i],MoveLength(state.movelist[i]));
    ...
    int MaxValue(State state, char[][] board, int alpha, int beta, int currDepth, int depthLimit){
        //if isLeaf then eval(State)
        if (currDepth == depthLimit) return eval(state);
        State newState= new State();
        newState.player= state.player == 1? 2 : 1;
        memcpy(newState.board, state.board);
        FindLegalMoves(newState);
        for (int i=0; i< newState.moveptr; i++){

             alpha= math.max(alpha, MinValue(newState, board, alpha, beta, ++currDepth, depthLimit));
             if (alpha >= beta) return beta;
        }
        return alpha;
    }

    int MinValue(State state, char[][] board, int alpha, int beta, int currDepth, int depthLimit){
        if (currDepth == depthLimit) return eval(state);
        State newState=new State();
        newState.player= state.player == 1? 2 : 1;
        FindLegalMoves(newState);
        ...
        for(int i=0; i< newState.moveptr; i++){
            memcpy(newState.board, state.board);
            // How do we make a move to update newState
            memset(move, i,12);
            PerformMove(newState.board, move, );
            beta= math.min(beta, MaxValue(newState, board, alpha, beta, ++currDepth, depthLimit));
